...
<div class="max-w-4xl mx-auto px-6 pt-44 pb-20 min-h-screen bg-slate-50">
    ...
    <form action="{{ route('questions.store') }}" method="POST">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-12">
            ...
            <div class="bg-indigo-600 rounded-[2.5rem] shadow-lg p-8 border-4 border-white">
            ...
            {{-- Section Durasi Waktu (BARU) --}}
            <div class="bg-amber-500 rounded-[2.5rem] shadow-lg p-8 border-4 border-white flex flex-col justify-center text-white">
                <label class="block text-xs font-black text-amber-100 mb-3 tracking-[0.2em]">2. DURASI WAKTU (MENIT)</label>
                <div class="flex items-center gap-4 bg-white/10 p-2 rounded-2xl border-2 border-white/20">
                    <div class="bg-white p-3 rounded-xl shadow-inner text-amber-600">
                        <svg xmlns="http://www.w3.org/2000/svg" class="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2.5" d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z" />
                        </svg>
                    </div>
                    <input type="number" name="durasi" value="60" min="1"
                           class="w-full bg-transparent border-none outline-none font-black text-2xl placeholder-white/60 text-white"
                           placeholder="60">
                </div>
                <p class="mt-3 text-[10px] font-bold text-amber-100 italic">*Setiap kelas mungkin butuh waktu berbeda</p>
            </div>
        </div>

        {{-- Looping 30 Soal --}}
        @for ($i = 1; $i <= 30; $i++)
        <div class="bg-white rounded-[3rem] shadow-sm p-10 border border-slate-200 mb-10 relative overflow-hidden transition-all hover:shadow-md group">
            ...
            <div class="flex items-center gap-3 mb-8">
                ...
                <div class="h-[2px] flex-1 bg-slate-100 rounded-full group-hover:bg-indigo-100 transition-all"></div>
                <span class="text-[10px] font-black text-slate-400 uppercase tracking-widest">Question Item</span>
            </div>

            {{-- Isi Pertanyaan --}}
            <div class="mb-8">
                <label class="block text-xs font-black text-slate-400 uppercase tracking-widest mb-3 italic">Tulis Pertanyaan</label>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-5 mb-10">
                ...
                <div class="group/option">
                    <div class="flex items-center bg-slate-50 rounded-2xl border-2 border-slate-50 group-focus-within/option:border-indigo-500 group-focus-within/option:bg-white transition-all overflow-hidden">
